...
@test "tvdb-search-series UNIT : where no series search term provided should error" {
	run "$UTILITIES_SRC_DIR"/tvdb-get-series-episode-details
	assert_failure
	assert_output "ERROR: No series search term provided."
}

@test "tvdb-search-series INT : where series found and season number and episode number provided but episode doesnt exist should return null" {
	run "$UTILITIES_SRC_DIR"/tvdb-get-series-episode-details "Star Trek The Next Generation" 1 78
	assert_success
	assert_output "null"
}

@test "tvdb-search-series INT : where series found and season number and episode number provided should return episode details" {
	run "$UTILITIES_SRC_DIR"/tvdb-get-series-episode-details "Star Trek The Next Generation" 3 7
	assert_success
	assert_output "[{\"season\":3,\"episode\":7,\"name\":\"The Enemy\"}]"
}

@test "tvdb-search-series INT : where series found and season number provided should return episode details for all episodes in season" {
	run "$UTILITIES_SRC_DIR"/tvdb-get-series-episode-details "Sherlock on Masterpiece" 2
	assert_success
...
@test "tvdb-search-series INT : where series found and no season or episode number provided should return episode details for all episodes in all seasons" {
	run "$UTILITIES_SRC_DIR"/tvdb-get-series-episode-details "Firefly"
	assert_success
...
@test "tvdb-search-series INT : where multiple series found for search term should request input from user as to which series was requested" {
